...
Rule::Rule(std::string&& name, Modifier mod, std::vector<Meta>&& metas, std::shared_ptr<StringsTrie>&& strings,
		ASTNode::Ptr&& condition, std::vector<std::string>&& tags)
...
std::string Rule::getText() const
{
	...
	ss << "\tcondition:\n" << indent << getCondition()->getExpression()->getText(indent) << "\n}";
	return ss.str();
}
 ...
 */
const ASTNode::Ptr& Rule::getCondition() const
{
	return _condition;
}
 ...
 * @param condition Condition expression.
 */
void Rule::setCondition(const ASTNode::Ptr& condition)
{
