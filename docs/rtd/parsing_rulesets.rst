...
``getIterable()`` (``iterable`` in Python) to return an iterated set (can also be ``them``) and ``getBody()`` (``body`` in Python) to return the body of a for expression. For ``OfExpression`` and ``OfInRangeExpression``, the ``getBody()`` method always returns ``nullptr`` (``None`` in Python).
  ...
  * ``OfInRangeExpression`` - refers to ``of`` (``all of ($str1, $str2) in (filesize-500..filesize)``)
